...
fprintf('\n--- Selected Variables ---\n');
for i = sel_idx
    fprintf('Variable %d = %.3f (freq = %.2f)\n', i, betafinal(i), freq(i));
end
...
% 2) 标准化测试集数据 (确保标准化方式与训练集相同)
Xtest_log = log(Ztest + 0.5);  % 处理微生物数据
Xtest_part = [csvread('test_cov_two.csv'), Xtest_log];  % 组合测试集 X
[Xtest_std, muX_test, stdX_test] = zscore(Xtest_part);

% 3) 选择训练集中选中的变量
Xtest_selected = Xtest_std(:, sel_idx);  

% 4) 计算测试集的 Y 预测值 (反标准化)
Ytest_pred_std = Xtest_selected * tembeta;  
Ytest_pred = stand.muy + stand.Sy * Ytest_pred_std;

% 5) 计算测试集的真实 Y (反标准化)
Ytest_true = Ytest .* stand.Sy + stand.muy;

% 6) 计算测试集误差 (MSE)
TestMSE = mean((Ytest_true - Ytest_pred).^2);
fprintf('Test MSE = %.3f\n', TestMSE);
...
% 预测 vs 真实值散点图
figure;
plot(Ytest_true, Ytest_pred, 'o', 'MarkerSize', 8);
refline(1, 0);
xlabel('Observed BMI');
ylabel('Predicted BMI');
title('Observed vs Predicted BMI (Test Set)');
grid on;

% 变量选择频率
temp = 1:p_new; 
figure;
plot(temp, freq, 'ko', 'MarkerSize', 5);
hold on;
stem(sel_idx, top, '-r', 'LineWidth', 1.5);
hold off;
