...
mcmc_spike_slab_microbiome <- function(
  ...
  for (iter in seq_len(niter)) {
    ...
    for (jj in seq_len(J)) {
      # Log-marginal-likelihood difference for zeta=1 vs zeta=0
      ...
      # Constrain log-odds for numerical stability
      logodds <- max(min(logodds, 15), -15)
      p_incl <- 1 / (1 + exp(-logodds))
      zeta[jj] <- rbinom(1, 1, p_incl)
      
      # If zeta_j = 1, sample phi_j from its posterior; otherwise set to 0
      if (zeta[jj] == 1) {
